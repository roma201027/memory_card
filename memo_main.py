...
def testlist():
    ...
    frm = Form('Який ресурс потрібен для створення залізних інструментів?', 'Залізо',
               'Вугілля', 'Алмази', 'Камінь')
    questions_listmodel.form_list.append(frm)

    frm = Form('Який матеріал використовується для створення маяка?', 'Скло',
               'Вода', 'Обсидіан', 'Дерево')
    questions_listmodel.form_list.append(frm)

    frm = Form('Що потрібно для створення порталу в Край?', 'Око Ендермена',
               'Залізо', 'Золото',
               'Динаміт')
    questions_listmodel.form_list.append(frm)

    frm = Form('Як називається моб, який живе в Краю?', 'Ендермен',
               'Зомбі', 'Павук', 'Скелет')
    questions_listmodel.form_list.append(frm)

    frm = Form('Який інструмент найкраще підходить для копання піску?', 'Лопата',
               'Сокира', 'Меч', ' Кірка')
    questions_listmodel.form_list.append(frm)

    frm = Form('Який ресурс можна знайти лише у Нижньому світі?', 'Незерит',
               'Камінь', 'Гравій', 'Лапіс-лазурит')
    questions_listmodel.form_list.append(frm)

    frm = Form('Як звати босового моба, який знаходиться в Краю?', 'Дракон Краю',
               ' Скелет-іссушувач', 'Зомбі-Гігант', 'Крипер')
    questions_listmodel.form_list.append(frm)

    frm = Form('Що використовується для створення динаміту?', 'Порох',
               'Алмази', 'Пісок', 'Камінь')
    questions_listmodel.form_list.append(frm)

    frm = Form('Який ресурс є найміцнішим для створення броні?', 'Незерит',
               'Золото', 'Шкіра',
               'Залізо')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка тварина дає вовну у грі Minecraft?', 'Вівця',
               'Корова', 'Павук', 'Кролик')
    questions_listmodel.form_list.append(frm)
